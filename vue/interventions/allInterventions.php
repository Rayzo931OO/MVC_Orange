<?php
echo "<br>";
//bouton Filtrer pour priorite
echo ' <form class="formulaire" action="index.php" method="post">
    <div>
        <label for="type">Priorite</label></br>
        <select name="priorite" id="priorite" required>
            <option value="Mineur">Urgent</option>
            <option value="Urgente">Mineur</option>
        </select>
    </div>';
echo '<div>
        <button type="submit" name="priorite_filter" value="priorite_filter" />
        Filtrer
        </button>
    </div>
</form>';
...
if (count($interventions) > 0) {
    ...
    foreach ($interventions as $intervention) {
        ...
        echo "<td>" . $intervention['date_inter'] . "</td>
          ...
          <td>" . $intervention['priorite'] . "</td><td class='actions'>";
        ...
        if($_SESSION["role"] == "admin" || $_SESSION["role"] == "superviseur" || $_SESSION["role"] == "client"){
    ...
    if (count($unAssignedInterventions) > 0) {
        ...
        foreach ($unAssignedInterventions as $unAssignedIntervention) {
            ...
            echo "<td>" . $unAssignedIntervention['date_inter'] . "</td>
              ...
              <td>" . $unAssignedIntervention['description'] . "</td>
              <td>" . $unAssignedIntervention['priorite'] . "</td><td class='actions'>";
        ...
        // j'aimerais que tu me afficher un tableau qui dans un premier temps affiche les noms et prénoms des techniciens et d'avoir le nombre d'interventions en cours et urgentes par technicien en utilisant cette fonction selectInterventionUrgentes()
...
if ($_SESSION["role"] == "superviseur" || $_SESSION["role"] == "admin") {
    // var_dump($rapport);
    echo "<table class='tableau' border-collapse='collapse' >

    <caption class='caption'>Listes des interventions urgentes</caption>
    <thead>
        <tr>
            <th>Technicien</th>
            <th>Nombre d'interventions en cours et urgentes</th>";
    ...
    foreach ($rapport as $rap) {
        echo "<tr>
          <td>" . $rap['nom'] . " " . $rap['prenom'] . "</td>
          <td>" . $rap['nbInterventions'] . "</td>";
